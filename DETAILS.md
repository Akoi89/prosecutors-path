...
- **Evidence descriptions and Logic cards that overflow their box**: official English
  ...
  (Japanese, Capcom, condensed, fan) from your own extracted data. Together with the earlier exceptions that makes **135
  rows where Capcom's wording is edited**: the 108 in `tools/condense.py` (105 generated,
  plus 3 hand-written entries that take precedence over their generated versions), 20
  over-wide card titles in `tools/plates.py` (`TRIMS`, 8 of them until 1.6.3 widened the
  set), and 7 renamed rows in `tools/names.py` (`ROWFIX`).
...
| `audits/audit_typography.py` | Not a width check, a SHAPE check: a spurious space, a line opening with a comma, empty brackets, a stray blank line. None of those make a line too wide, which is how the v1.9.0 development build put a space after 144 re-opened brackets and survived a full review. Comparative against an older ROM, since several of these quirks predate us; `--selftest` proves it can fail against eight broken fixtures |
